...
document.addEventListener("DOMContentLoaded", function () {
  document.getElementById('currencyPair').addEventListener('change', loadWatchlist);
  ...
  function loadWatchlist() {
    ...
    Object.keys(selectedCoins).forEach((symbol) => {
      ...
      row.innerHTML = `
          <td>${coin.name}</td>
          <td>${symbol}</td>
          <td>${coin.price.toLocaleString("id-ID", { style: "currency", currency: "IDR" })}</td>
          <td>${coin.price1h}</td>
          <td>${coin.price2h}</td>
          <td>
              <button onclick="window.location.href='detailcoin.html?symbol=${symbol}&name=${coin.name}'" class="detail-btn">Detail</button>
